...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        if (pairKey && pairKey.length !== 8) {
                        throw new boom_1.Boom('Custom pairing code must be exactly 8 characters', { statusCode: 400 });
                }
                const phoneUser = String(phoneNumber).split('@')[0].replace(/[^0-9]/g, '');
                if (!phoneUser) {
                        throw new boom_1.Boom('A valid WhatsApp phone number or JID is required', { statusCode: 400 });
                }
                ...
                authState.creds.me = {
                        id: (0, WABinary_1.jidEncode)(phoneUser, 's.whatsapp.net'),
                        name: '~'
                };
    ...
    ws.on('close', (code, reason) => {
        logger.warn({ code, reason: reason === null || reason === void 0 ? void 0 : reason.toString() }, 'websocket closed by server');
        return end(new boom_1.Boom(`Connection Terminated (${code})`, {
            statusCode: Types_1.DisconnectReason.connectionClosed,
            data: { code, reason: reason === null || reason === void 0 ? void 0 : reason.toString() }
        }));
    });
